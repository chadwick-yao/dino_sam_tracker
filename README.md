...
pip install --no-cache-dir git+https://github.com/huggingface/transformers.git@31f9a289a6207be6cae746e009d8e0db523be203
```

Modify processing_grounding_dino.py with following content:
```python
# coding=utf-8
...
def get_phrases_from_posmap(
    ...
    if posmap.dim() == 1:
        ...
        posmap[left_idx + 1 : right_idx] = True
        non_zero_idx = posmap.nonzero(as_tuple=True)[0].tolist()
...
class GroundingDinoProcessor(ProcessorMixin):
    ...
    def post_process_grounded_object_detection(
        ...
        for idx, (s, b, p) in enumerate(zip(scores, boxes, probs)):
            ...
            sep_idx = [
                i
                for i in range(len(input_ids[idx]))
                if input_ids[idx][i] in [101, 102, 1012]
            ]
            labels = []
            for item in prob:
                max_idx = item.argmax()
                ...
                label_ids = get_phrases_from_posmap(item > text_threshold, input_ids[idx], left_idx, right_idx)
                label = self.batch_decode(label_ids)
                labels.append(label[0])
            results.append({"scores": score, "labels": labels, "boxes": box})
